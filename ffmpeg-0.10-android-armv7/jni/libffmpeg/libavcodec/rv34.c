...
//#define DEBUG
#define BAOFENG_OPT 1

#ifdef BAOFENG_OPT
static int filter_open = 1;
static int skip_count = 0;

static inline void check_skip_filter(int skip_level)
{
	...
	filter_open = skip_count>0 ?  0:1;
	//dlog1("xiaodong, skip:%d, filter_open:%d\n", skip_level, filter_open);
}
#endif
...
// __YUBO
		#ifdef BAOFENG_OPT
            if(r->loop_filter && s->mb_y >= 2 && filter_open)
		#else
                            ...
                            AVPacket *avpkt)
{
	...
	#ifdef BAOFENG_OPT
	check_skip_filter(avctx->skip_frame);
	#endif
